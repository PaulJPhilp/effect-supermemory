...
const createMemoryStreamClientLayer = (
  ...
  return MemoryStreamClient.Default(config).pipe(
...
describe("MemoryStreamClient", () => {
  ...
  it.skipIf(!hasRealApiKey)(
    "listAllKeys streams keys correctly from NDJSON",
    async () => {
  ...
  it.skipIf(!hasRealApiKey)(
    "streamSearch streams SearchResults correctly from NDJSON",
    async () => {
      const program = Effect.gen(function* () {
        const client = yield* MemoryStreamClient;
  ...
  it.skipIf(!hasRealApiKey)(
    "listAllKeys fails on initial 401 AuthorizationError",
    async () => {
  ...
  it.skipIf(!hasRealApiKey)(
    "streamSearch handles empty results stream",
    async () => {
      const program = Effect.gen(function* () {
        const client = yield* MemoryStreamClient;
        const stream = yield* client.streamSearch(
  ...
  it.skipIf(!hasRealApiKey)(
    "listAllKeys handles stream interruption gracefully",
    async () => {
      const program = Effect.gen(function* () {
        const client = yield* MemoryStreamClient;
        const stream = yield* client.listAllKeys();
