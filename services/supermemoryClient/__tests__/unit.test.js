...
const createSupermemoryClientLayer = (configOverrides) => {
              ...
              response instanceof Error || response._tag === "Failure"
                ...
                : response.status === 404
                  ...
                  : response.status !== 200
                    ...
                    : Effect.succeed(Utils.fromBase64(response.body.value))
            )
          ),
        delete: (key) =>
          Effect.sync(() =>
  ...
  it("fails after exhausting retries for persistent 5xx error", async () => {
    ...
    if (result._tag === "Failure") {
      ...
      if (Option.isSome(error)) {
        ...
        expect(errorValue.message).toContain(
  ...
  it("does not retry on 401 (put), fails immediately", async () => {
    ...
    if (result._tag === "Failure") {
      ...
      if (Option.isSome(error)) {
        const errorValue = error.value;
        expect(errorValue).toBeInstanceOf(MemoryValidationError);
        expect(errorValue.message).toContain("Authorization failed");
      }
    }
  });
