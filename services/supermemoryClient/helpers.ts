...
export const safeToBase64 = (str: string): Effect.Effect<string, Error> =>
	Effect.try({
		try: () => toBase64(str),
		catch: (e) => new Error(`Failed to encode to base64: ${e instanceof Error ? e.message : String(e)}`),
	});
...
export const safeFromBase64 = (b64: string): Effect.Effect<string, Error> =>
	Effect.try({
		try: () => fromBase64(b64),
		catch: (e) => new Error(`Failed to decode from base64: ${e instanceof Error ? e.message : String(e)}`),
	});
